...
ggsave(here::here("analyze", "plots", "fig_s3.pdf"), plot = fig_clim_all, width = 11, height = 8.5, units = "in", dpi = 1000) # for showing all data
...
ggsave(here::here("analyze", "plots", "fig_1.pdf"), plot = tree, width = 8.5, height = 11, scale = 1.25, dpi = 1000)
...
h_bonds <- aa %>%
  ...
  mutate(across(c(AA, AA_Ancestral),
                .fns = ~ case_when(. %in% ALIPHATIC ~ "aliphatic",
                                   ...
                                   . %in% AMIDIC ~ "amidic"),
                .names = "{.col}_Type")) %>%
  mutate(Significant = ifelse(Site %in% sig_sites, TRUE, FALSE)) %>%
...
cowplot::save_plot(here::here("analyze", "plots", "fig_s2.pdf"), plot = tree_heatmap, base_width = 11, base_height = 11, dpi = 1000)
```
...
ggsave(here::here("analyze", "plots", "fig_s1.pdf"), plot = gene_tree_plot, width = 8.5, height = 11, dpi = 1000)
...
biome_aa_chisq <- biome_aa %>%
  ...
  mutate(ChiSq = purrr::map(ContingencyTable, ~rstatix::chisq_test(., simulate.p.value = TRUE))) %>%
  tidyr::unnest(ChiSq) %>%
...
ggsave(here::here("analyze", "plots", "fig_s5.pdf"), biome_plot, width = 7, height = 8, dpi = 1000)
...
range_plot <- ggpubr::ggballoonplot(range_aa_summary,
  ...
  theme(axis.line = element_blank(),
        ...
        strip.background = element_blank()) +
  facet_wrap(~ SiteP, scales = "free_x")
ggsave(here::here("analyze", "plots", "fig_s6.pdf"), range_plot, width = 7, height = 10, dpi = 1000)
